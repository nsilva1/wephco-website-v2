...
export default function EventsClient({ events }: EventsClientProps) {
  const [activeTab, setActiveTab] = useState<'upcoming' | 'virtual' | 'past'>(
    'upcoming'
  );
  ...
  const filteredEvents = events.filter((evt) => {
    if (activeTab === 'upcoming')
      return !evt.isPast && evt.format === 'Physical';
    ...
    <div className="relative min-h-screen bg-background-dark text-slate-100 font-sans pt-20">
      ...
      <section className="relative z-10 max-w-7xl mx-auto px-6 py-16 text-center space-y-6">
        ...
        <h1 className="text-4xl md:text-6xl font-black text-slate-100 tracking-tight leading-none">
          Wephco{' '}
          <span className="bg-linear-to-r from-primary to-amber-500 bg-clip-text text-transparent italic">
            Events
          </span>
        </h1>

        <p className="text-slate-400 text-sm md:text-lg max-w-2xl mx-auto font-light leading-relaxed">
          Access private launch exhibitions, secure offshore real estate
          seminars, and virtual immersive walkthroughs configured for
          high-society portfolio builders.
        </p>
      ...
      <section className="relative z-10 max-w-7xl mx-auto px-6 pb-24">
        {filteredEvents.length > 0 ? (
          ...
          <div className="text-center py-20 bg-slate-900/30 border border-primary/5 rounded-2xl max-w-xl mx-auto">
            <Compass
              className="size-12 text-primary/40 mx-auto mb-4 animate-spin"
        ...
        <div className="max-w-5xl mx-auto px-6 grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
          <div className="space-y-4 text-center md:text-left">
            ...
            <p className="text-xs text-slate-400 leading-relaxed">
              Every Wephco event is organized under absolute security protocols.
              High-profile attendees can arrange airport transfers, discrete
              transport, and personalized accommodation with our dedicated
              concierge desk.
            </p>
          </div>
          <div className="p-6 bg-slate-900/40 border border-primary/10 rounded-2xl flex flex-col gap-4 text-center md:text-left">
            <h4 className="text-sm font-bold text-primary tracking-widest uppercase">
              Immediate Desk Support
            </h4>
            <p className="text-[11px] text-slate-400">
              Our real estate concierge is available to handle priority
