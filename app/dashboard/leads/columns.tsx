    ...
    cell: ({ row }) => {
      ...
      return property ? (
        <Link
          ...
          className="text-blue-600 hover:underline text-sm max-w-[180px] truncate block">
          {property.title}
        </Link>
      ) : (
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <Badge
          variant="secondary"
          className={`capitalize ${styles[status?.toLowerCase()] || 'bg-gray-400 text-white'}`}>
          {status || 'New'}
        </Badge>
      );
    },
