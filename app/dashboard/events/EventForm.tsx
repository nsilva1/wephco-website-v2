...
export default function EventForm({ event, mode }: EventFormProps) {
  ...
  const addHost = () => {
    if (
      newHost.name.trim() &&
      newHost.role.trim() &&
      (newHost.imagePreview || newHost.imageFile)
    ) {
      setHosts((prev) => [
        ...prev,
        {
      ...
      toast.error(
        'Please fill in all host details, including picking an image.'
      );
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload: EventItem = {
        ...
        seatsRemaining: formState.seatsRemaining
          ? Number(formState.seatsRemaining)
          : undefined,
        ...
        galleryImages:
          finalGalleryImages.length > 0 ? finalGalleryImages : undefined,
      };
                ...
                {highlights.length > 0 && (
                  <ul className="space-y-2 max-h-40 overflow-y-auto pr-1">
                    {highlights.map((h, idx) => (
                      <li
                        key={idx}
                        className="flex items-center justify-between p-2 bg-slate-50 border rounded-lg text-sm text-slate-850">
                        <span>{h}</span>
                        <Button
                          type="button"
                          variant="ghost"
              ...
              <CardContent className="space-y-4">
                <div className="space-y-3 p-3 bg-slate-50 border rounded-lg">
                  ...
                  <Button
                    ...
                    className="w-full bg-[#cfb53b] hover:bg-[#b89e2f] text-white">
                    Add Agenda Item
                  </Button>
                </div>
                ...
                {agenda.length > 0 && (
                  <div className="space-y-2 max-h-56 overflow-y-auto pr-1">
                    {agenda.map((item, idx) => (
                      <div
                        key={idx}
                        className="flex justify-between items-start p-3 bg-white border rounded-lg text-xs gap-3">
                        <div className="flex-1 space-y-1">
                          <div className="font-semibold text-slate-800 flex items-center gap-1.5">
                            <span className="text-[#cfb53b] font-medium">
                              {item.time}
                            </span>
                            <span>-</span>
                ...
                {hosts.length > 0 && (
                  <div className="space-y-2 max-h-56 overflow-y-auto pr-1">
                    {hosts.map((host, idx) => (
                      <div
                        key={idx}
                        className="flex justify-between items-center p-2 bg-white border rounded-lg text-xs gap-3">
                        <div className="flex items-center gap-3">
                          {host.imagePreview && (
                            <div className="relative h-10 w-10 rounded-full overflow-hidden shrink-0 border">
                              <Image
                                src={host.imagePreview}
                                alt={host.name}
                                fill
                                className="object-cover"
                              />
                          ...
                          <div>
                            ...
                            <p className="text-slate-500">{host.role}</p>
                          </div>
                        </div>
                        <Button
                          type="button"
                ...
                {(existingGallery.length > 0 || newGalleryFiles.length > 0) && (
                  <div className="grid grid-cols-3 gap-2 max-h-64 overflow-y-auto p-1 border rounded-lg">
                    ...
                    {newGalleryFiles.map((gf, idx) => (
                      <div
                        key={`new-${idx}`}
                        className="relative aspect-square rounded overflow-hidden group border-2 border-[#cfb53b]">
                        <Image
                          src={gf.preview}
