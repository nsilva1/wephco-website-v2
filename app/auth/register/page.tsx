...
const RegisterPage = () => {
  ...
  const handleRegisterWithCode = async () => {
    ...
    try {
      ...
      if (
        ['SUPPORT', 'ADMIN', 'SUPERADMIN', 'AGENT'].includes(
    ...
    <div className="relative min-h-screen flex flex-col font-display">
        ...
        <DialogContent className="sm:max-w-md bg-background-dark/95 border border-primary/20 text-white backdrop-blur-xl">
          <DialogHeader>
            ...
            <DialogDescription className="text-slate-400 text-xs">
              Please enter the authentication code to complete your
              registration.
            </DialogDescription>
          </DialogHeader>
          <div className="flex flex-col space-y-4 py-4">
            ...
            <div className="space-y-2">
              <label
                htmlFor="authCode"
                className="block text-[10px] uppercase font-bold text-primary tracking-widest">
                Code
              </label>
              <input
                id="authCode"
      ...
      <header className="relative z-10 flex items-center justify-between px-6 py-6 md:px-20 lg:px-40">
        <Link href="/" className="flex items-center gap-3 text-primary">
          ...
          />
          <h2 className="text-slate-100 text-lg font-extrabold tracking-tight uppercase hidden sm:block">
            WEPHCO
          </h2>
        </Link>
        <div className="flex items-center gap-4">
          ...
          <Link
            href="/auth/login"
            className="flex items-center justify-center rounded-lg h-10 px-6 border border-primary/50 text-primary text-xs font-bold hover:bg-primary/10 transition-colors uppercase tracking-wider">
            Login
          </Link>
        </div>
      </header>
      ...
      <main className="relative z-10 flex-1 flex items-center justify-center px-4 py-8">
        <div className="w-full max-w-[500px] bg-background-dark/50 backdrop-blur-xl border border-white/10 rounded-xl p-8 md:p-10 shadow-2xl">
          <div className="mb-8 text-center">
            <h1 className="text-white text-3xl font-black leading-tight tracking-tight mb-2">
              Create Account
            </h1>
            <p className="text-slate-400 text-xs">
              Enter the world of exclusive high-end real estate.
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {error && (
              <div
                role="alert"
                className="p-3 rounded-md border border-red-900 bg-red-950/40 text-red-200 text-xs">
                {error}
              </div>
            )}
            {successMessage && (
              <div
                role="status"
                className="p-3 rounded-md border border-green-900 bg-green-950/40 text-green-200 text-xs">
                {successMessage}
              </div>
            )}
      ...
      <footer className="relative z-10 py-6 text-center">
        <p className="text-slate-500 text-[9px] uppercase tracking-[0.2em] font-bold">
          © {new Date().getFullYear()} Wephco. All Rights Reserved.
        </p>
      </footer>
    </div>
  );
