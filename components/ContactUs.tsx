...
import { IContactUs, IConsultation } from '@/interfaces/userInterface'


const ContactUs = ({contact = true}: {contact?: boolean}) => {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (!name || !email || !phoneNumber || (!contact && !message)) {
        ...
        const data = contact ? contactUsData : consultationData
        ...
        try {
            if (contact) {
                ...
                await createConsultation(data as IConsultation)
            }
                ...
                <div className='bg-white rounded-xl border border-gray-300 w-96 md:w-[500px] px-8 py-16'>
                    ...
                    <form onSubmit={handleSubmit}>
                        <fieldset className='flex flex-col gap-8'>
                            ...
                            {
                                !contact && <textarea value={message} onChange={(e) => setMessage(e.target.value)} className='border-b p-2 w-full' placeholder='Message...' rows={4} required></textarea>
                            }
